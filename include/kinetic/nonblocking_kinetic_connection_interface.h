...
class SimpleHandler : public HandlerInterface {
  public:
    ...
    void Handle(const Command &response,
  ...
  private:
    const shared_ptr<SimpleCallbackInterface> callback_; DISALLOW_COPY_AND_ASSIGN(SimpleHandler);
};
...
class GetCallbackInterface {
  public:
    ...
    virtual void Success(const std::string &key,
                         std::unique_ptr<KineticRecord> record) = 0;
...
class GetHandler : public HandlerInterface {
  ...
  private:
    const shared_ptr<GetCallbackInterface> callback_; DISALLOW_COPY_AND_ASSIGN(GetHandler);
};
...
class GetVersionHandler : public HandlerInterface {
  ...
  private:
    const shared_ptr<GetVersionCallbackInterface> callback_; DISALLOW_COPY_AND_ASSIGN(GetVersionHandler);
};
...
class GetKeyRangeHandler : public HandlerInterface {
  ...
  private:
    const shared_ptr<GetKeyRangeCallbackInterface> callback_; DISALLOW_COPY_AND_ASSIGN(GetKeyRangeHandler);
};
...
class MediaScanHandler : public HandlerInterface {
  ...
  private:
    const shared_ptr<MediaScanCallbackInterface> callback_; DISALLOW_COPY_AND_ASSIGN(MediaScanHandler);
};
...
class MediaOptimizeHandler : public HandlerInterface {
  ...
  private:
    const shared_ptr<MediaOptimizeCallbackInterface> callback_; DISALLOW_COPY_AND_ASSIGN(MediaOptimizeHandler);
};
...
class PutHandler : public HandlerInterface {
  ...
  private:
    const shared_ptr<PutCallbackInterface> callback_; DISALLOW_COPY_AND_ASSIGN(PutHandler);
};
...
class GetLogHandler : public HandlerInterface {
  ...
  private:
    const shared_ptr<GetLogCallbackInterface> callback_; DISALLOW_COPY_AND_ASSIGN(GetLogHandler);
};
...
class P2PPushCallbackInterface {
  public:
    ...
    virtual void Failure(KineticStatus error,
                         Command const *const response) = 0;
};
...
class P2PPushHandler : public HandlerInterface {
  ...
  private:
    const shared_ptr<P2PPushCallbackInterface> callback_; DISALLOW_COPY_AND_ASSIGN(P2PPushHandler);
};
...
class NonblockingKineticConnectionInterface {
  public:
    ...
    virtual bool Run(fd_set *read_fds,
                     fd_set *write_fds,
                     int *nfds) = 0;
    ...
    virtual HandlerKey Get(const string key,
    ...
    virtual HandlerKey MediaScan(const shared_ptr<const string> start_key,
                                 bool start_key_inclusive,
                                 const shared_ptr<const string> end_key,
                                 bool end_key_inclusive,
                                 int32_t max_results,
                                 const shared_ptr<MediaScanCallbackInterface> callback) = 0;

    virtual HandlerKey MediaScan(const string start_key,
                                 bool start_key_inclusive,
                                 const string end_key,
                                 bool end_key_inclusive,
                                 int32_t max_results,
    ...
    virtual HandlerKey MediaOptimize(const shared_ptr<const string> start_key,
                                     bool start_key_inclusive,
                                     const shared_ptr<const string> end_key,
                                     bool end_key_inclusive,
                                     const shared_ptr<MediaOptimizeCallbackInterface> callback) = 0;

    virtual HandlerKey MediaOptimize(const string start_key,
                                     bool start_key_inclusive,
                                     const string end_key,
                                     bool end_key_inclusive,
                                     const shared_ptr<MediaOptimizeCallbackInterface> callback) = 0;
    ...
    virtual HandlerKey Delete(const string key,
